...
read.mtx.sparse <- function(mtx.file,
    ...
    if(is.null(memory.idx)) {
        if(!file.exists(memory.idx.file))
            mmutil_build_index(mtx.file, memory.idx.file)

        memory.idx <- mmutil_read_index(memory.idx.file)
    }

    if(is.null(sub.cols)) {
        .info <- mmutil_info(mtx.file)
...
vcat.sparse <- function(top.hdr, bottom.hdr, out.hdr){
    ...
    top.mtx <- read.mtx.sparse(top.mtx.file)
    bottom.mtx <- read.mtx.sparse(bottom.mtx.file)
