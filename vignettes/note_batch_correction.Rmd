...
pheatmap(.nmf0$theta[order(kk), ], Rowv=NA, Colv=NA, scales="row", cluster_rows = F)
```

```{r}
table(kk, apply(.nmf0$theta, 1, which.max))
```

With batch correction:

```{r fig.width=5, fig.height=5}
pheatmap(.nmf1$theta[order(kk), ], Rowv=NA, Colv=NA, scales="row", cluster_rows = F)
```

```{r}
table(kk, apply(.nmf1$theta, 1, which.max))
```
