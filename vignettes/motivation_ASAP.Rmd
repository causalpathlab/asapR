...
.out <- asap_fit_nmf_susie(Y, 7, max_iter=200, rseed=131, EPS=1e-2);

plot(.out$log.likelihood)

zz <- .out$theta[order(.membership), ]
heatmap(zz, Rowv=NA)

zz <- .out$log.rho[order(.membership), ]
heatmap(zz, Rowv=NA)

.out <- asap_fit_modular_nmf(Y, 7, 100, NUM_THREADS = 16)

zz <- .out$column$mean[order(.membership), -2]
heatmap(zz, Rowv=NA)
