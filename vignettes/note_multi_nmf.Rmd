...
.out0 <- asap_fit_nmf(cbind(y1, y2, y3, u0), maxK=7)
```
...
```{r}
u0 <- cbind(pmax(uu, 0), -pmin(uu, 0))
.out <-  asap_fit_nmf_shared_dict(list(y1, y2, y3, u0), maxK=7)
```

```{r fig.width=5, fig.height=4}
heatmap(.out$log.beta[gg,], Rowv=NA, Colv=NA, main = "beta")
```

```{r fig.width=5, fig.height=4}
heatmap(.out$log.theta[[1]][kk,], Rowv=NA, Colv=NA, main = "theta1")
```

```{r fig.width=5, fig.height=4}
heatmap(.out$log.theta[[2]][kk,], Rowv=NA, Colv=NA, main = "theta2")
```

```{r fig.width=5, fig.height=4}
heatmap(.out$log.theta[[3]][kk,], Rowv=NA, Colv=NA, main = "theta3")
```
